...
import { getSystemConfig } from './utils.mjs';
...
if (!process.env.hasOwnProperty('ENCRYPTION_KEY'))
	console.log(
		chalk.yellow('No ENCRYPTION_KEY, backup will not be encrypted.'),
	);
...
for (let s = 0; s < config.services.length; s++) {
	...
	message += message == '' ? '' : '\n';
	if (service.type == 'ssh') message += await runSSH(systemConfig, service);
		...
		console.error(
			chalk.red(`${chalk.bold(service.type)} type not supported.`),
		);
}
